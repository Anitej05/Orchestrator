...
def upgrade() -> None:
    """Upgrade schema — made idempotent so it can be re-run after partial failure."""
    ...
    if not table_exists('conversation_tags'):
    ...
    if not index_exists('ix_agent_capabilities_id', 'agent_capabilities'):
    ...
    if not index_exists('ix_agent_credentials_user_id', 'agent_credentials'):
        op.create_index(op.f('ix_agent_credentials_user_id'), 'agent_credentials', ['user_id'], unique=False)

    if not table_exists('conversation_plans'):
    ...
    if not index_exists('ix_conversation_search_thread_id', 'conversation_search'):
        op.create_index(op.f('ix_conversation_search_thread_id'), 'conversation_search', ['thread_id'], unique=False)
    if not index_exists('ix_conversation_search_user_id', 'conversation_search'):
        op.create_index(op.f('ix_conversation_search_user_id'), 'conversation_search', ['user_id'], unique=False)

    if not table_exists('conversation_tag_assignments'):
    ...
    if table_exists('connection_logs'):
    ...
    if column_exists('agent_usage_analytics', 'metrics'):
    ...
    if column_exists('agents', 'rating'):
        op.drop_column('agents', 'rating')
    if column_exists('agents', 'image_url'):
        op.drop_column('agents', 'image_url')
    if column_exists('agents', 'rating_count'):
        op.drop_column('agents', 'rating_count')

    if column_exists('conversation_analytics', 'metrics'):
        op.drop_column('conversation_analytics', 'metrics')
    if column_exists('workflow_execution_analytics', 'metrics'):
