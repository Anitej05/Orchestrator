...
if not CEREBRAS_API_KEY:
    logger.warning("CEREBRAS_API_KEY is not set. Will use Groq as primary for vision decisions.")
if not GROQ_API_KEY:
    logger.warning("GROQ_API_KEY is not set. Fallback to Groq will not be available.")

# Ensure at least one text model API key is available
if not CEREBRAS_API_KEY and not GROQ_API_KEY:
    raise RuntimeError("Neither CEREBRAS_API_KEY nor GROQ_API_KEY is set. At least one is required.")
...
def get_text_llm_with_fallback() -> tuple[ChatOpenAI, str]:
    """
    Get text-only LLM with fallback chain: Groq → Cerebras.
    Note: Groq is primary because Cerebras has compatibility issues with browser-use.
    Returns (llm, provider_name) tuple.
    """
    # Try Groq first (primary for browser-use compatibility)
    if GROQ_API_KEY:
        try:
            logger.info("Initializing text-only LLM with Groq (primary)")
            llm = ChatOpenAI(
            ...
            return llm, "Groq (openai/gpt-oss-120b)"
        except Exception as e:
            logger.warning(f"Failed to initialize Groq: {e}. Falling back to Cerebras...")
    
    # Fallback to Cerebras (may have compatibility issues with browser-use)
    if CEREBRAS_API_KEY:
        try:
            logger.info("Initializing text-only LLM with Cerebras (fallback)")
            logger.warning("Note: Cerebras may have compatibility issues with browser-use")
            llm = ChatOpenAI(
            ...
            return llm, "Cerebras (qwen-3-235b-a22b-instruct-2507)"
        except Exception as e:
            logger.error(f"Failed to initialize Cerebras: {e}")
            raise RuntimeError("All text-only LLM providers failed to initialize")
    
    raise RuntimeError("No text-only LLM API keys available")

async def decide_if_vision_needed(task: str) -> bool:
    """
    Use the text-only LLM to decide if vision capabilities are needed for the task.
    Tries Groq first (more reliable), falls back to Cerebras if Groq fails.
    Returns True if vision is required, False otherwise.
    """
...
Respond with ONLY "YES" if vision is needed, or "NO" if text-based interaction is sufficient."""
    ...
    # Try Groq first (more reliable for this use case)
    if GROQ_API_KEY:
        try:
            logger.info("Attempting vision decision with Groq (primary)")
            client = OpenAI(
                api_key=GROQ_API_KEY,
                base_url="https://api.groq.com/openai/v1"
            )
            ...
            logger.info(f"✅ Groq vision decision: {'YES' if needs_vision else 'NO'} - Response: {decision}")
            return needs_vision
            
        except Exception as e:
            logger.warning(f"Groq error: {e}. Falling back to Cerebras...")
    ...
    if CEREBRAS_API_KEY:
        ...
        for attempt in range(max_retries):
            try:
                logger.info(f"Attempting vision decision with Cerebras (attempt {attempt + 1}/{max_retries})")
                client = OpenAI(
                    api_key=CEREBRAS_API_KEY,
                    base_url="https://api.cerebras.ai/v1"
                )
                ...
                logger.info(f"✅ Cerebras vision decision: {'YES' if needs_vision else 'NO'} - Response: {decision}")
                return needs_vision
                ...
                if "429" in error_msg or "rate" in error_msg or "too_many_requests" in error_msg:
                    if attempt < max_retries - 1:
                        wait_time = 2 ** attempt
                        logger.warning(f"Cerebras rate limit hit. Waiting {wait_time}s before retry...")
                        await asyncio.sleep(wait_time)
                        continue
                    else:
                        logger.warning(f"Cerebras rate limit persists.")
                        break
                else:
                    logger.warning(f"Cerebras error: {e}")
                    break
    
    # If both failed or no API keys available, default to vision mode
    logger.warning("All text models failed or unavailable. Defaulting to vision mode for safety.")
    return True
...
async def process_browser_task(task_id: str, request: BrowserTask):
    ...
    try:
        ...
        # Get text-only LLM with fallback chain (Cerebras → Groq)
        text_llm, text_provider = get_text_llm_with_fallback()
        logger.info(f"[Task {task_id}] Text-only LLM provider: {text_provider}")
        
        # Initialize vision LLM via Ollama Cloud (used when vision is required)
        ...
        # Decide whether vision is needed based on task analysis
        needs_vision = await decide_if_vision_needed(request.task)
        
        # Select the appropriate LLM
        if needs_vision:
            logger.info(f"[Task {task_id}] Vision required - using qwen3-vl:235b-cloud")
            llm = vision_llm
            use_vision = True
            model_info = "vision model (qwen3-vl:235b-cloud)"
        else:
            logger.info(f"[Task {task_id}] Text-only mode - using {text_provider}")
            llm = text_llm
            use_vision = False
            model_info = f"text-only model ({text_provider})"
        ...
        result = BrowserResult(
            ...
            note=f"Used {model_info}. Screenshot files saved for later use."
        )
