...
Comprehensive PDF processing agent powered by ReAct loop.
...
logger = setup_mega_logger("PDFAgent")

_server = create_agent_server(
    agent_class=PDFAgent,
    agent_id="pdf_agent",
...
def run_agent() -> None:
    import uvicorn
    port = int(os.getenv("PDF_AGENT_PORT", "8055"))
    logger.info(f"Starting PDF Agent server on port {port}...")
