...
from .types import (
    AgentStatus, AgentRequest, AgentResponse, ExecutionContext,
    ExecutionPlan, ExecutionStep, RecoveryPlan, CapabilityResult
)
...
class BaseAgent(ABC):
    ...
    def _discover_capabilities(self):
        ...
        logger.debug(f"Discovered {len(self.capability_registry.list_all())} capabilities")
        ...
        while attempt < self.config.max_retries:
            try:
                ...
                if recovery_plan.should_retry:
                    ...
                    logger.info(f"Trying alternative approach: {recovery_plan.reasoning}")
                    return await self._execute_alternative_approach(request, recovery_plan)
    ...
    async def _execute_single_attempt(self, request: AgentRequest, 
                                      attempt: int) -> AgentResponse:
        """
...
Respond with structured JSON."""
    ...
    async def _llm_synthesize_response(self, results: List[CapabilityResult],
...
Respond with structured JSON."""
        ...
        class SynthesizedResponse(BaseModel):
            ...
            key_data: Optional[Dict[str, Any]] = Field(description="Important data points")
    ...
    async def _llm_plan_recovery(self, error: Exception, request: AgentRequest,
...
Respond with structured JSON."""
        ...
        class RecoveryDecision(BaseModel):
            ...
            fixes: Optional[Dict[str, Any]] = Field(description="Parameter adjustments for retry")
            alternative: Optional[str] = Field(description="Alternative capability for alternate strategy")
            user_question: Optional[str] = Field(description="Question for user if escalating")
    ...
    async def _apply_recovery_fixes(self, request: AgentRequest,
                                     recovery_plan: RecoveryPlan) -> AgentRequest:
        """
    ...
    async def _execute_alternative_approach(self, request: AgentRequest,
                                            recovery_plan: RecoveryPlan) -> AgentResponse:
        """
        Execute alternative approach using different capabilities.
        """
        logger.info(f"Executing alternative approach: {recovery_plan.alternative_capability}")
