# agents/mail_agent/memory.py
from typing import List, Dict, Any, Optional
from datetime import datetime

class AgentMemory:
    """
    Simple in-memory state management for the Gmail Agent.
    ...
    def get_last_search_results(self, user_id: str) -> Optional[List[str]]:
        """Get the message IDs from the last search."""
        return self.get_context(user_id, "last_search_results")
    
    def save_draft_context(self, user_id: str, draft_id: str, context: Dict[str, Any]):
        """Save context for a draft (original message, thread, etc.)"""
        drafts = self.get_context(user_id, "drafts") or {}
        drafts[draft_id] = context
        self.update_context(user_id, "drafts", drafts)
    
    def get_draft_context(self, user_id: str, draft_id: str) -> Optional[Dict[str, Any]]:
        """Get context for a specific draft"""
        drafts = self.get_context(user_id, "drafts") or {}
