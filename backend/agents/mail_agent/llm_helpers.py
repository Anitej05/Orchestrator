# agents/mail_agent/llm_helpers.py
"""
Mail Agent - LLM Helper Functions

Domain-specific LLM methods for email operations.
...
class MailLLMHelpers:
    """
    Mail-specific LLM helpers.
    
    Mix this into MailAgent to get mail-specific LLM methods.
...
"""
    ...
    async def summarize_text_batch(self, texts: List[str]) -> str:
        """Summarize a batch of texts with urgency analysis"""
        if not texts:
...
JSON:"""
    ...
    async def analyze_attachment_importance(self, email_body: str, attachment_names: List[str]) -> Dict[str, Any]:
        """Analyze if attachments are critical to understanding the email"""
    ...
    async def check_ambiguity(self, query: str) -> Dict[str, Any]:
        """Analyze user query for genuine ambiguity in email context"""
...
{{
  ...
  "reasoning": "Internal logic for the decision"
    ...
    async def decompose_complex_request(self, prompt: str, error_context: Optional[str] = None) -> Dict[str, Any]:
        """Decompose complex email requests into executable steps"""
