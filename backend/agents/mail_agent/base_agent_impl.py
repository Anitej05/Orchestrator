...
class SmartResolver:
    ...
    async def resolve_message_ids(
        ...
        for identifier in identifiers:
            ...
            try:
                search_results = await self.gmail.semantic_search(identifier, 5, user_id)
...
class MailAgent(BaseAgent):
    """
    Complete Gmail automation agent with full feature set.
    """
    
    def __init__(self, agent_id: str = "mail_agent", services: Optional[AgentServices] = None):
        super().__init__(
            agent_id=agent_id,
    ...
    async def search_emails(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if not any(op in query for op in ["from:", "to:", "subject:", "label:", "is:"]):
                optimized_query = await self.llm_client.generate_optimized_query(query)
                query = optimized_query
            
            result = await self.gmail_client.semantic_search(query, max_results, user_id)
    ...
    async def read_email(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
                ...
                if analyze_attachments and message_data.get("attachments"):
                    for attachment in message_data["attachments"]:
                        if attachment.get("size", 0) < 5 * 1024 * 1024:
                            attachment["ai_analysis"] = "Attachment available for download"
    ...
    async def send_email(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            result = await self.gmail_client.send_email_with_attachments(
                ...
                is_html=is_html, attachment_file_ids=attachment_ids, user_id=user_id
            )
    ...
    async def draft_reply(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if not result.get("success"):
                return {"success": False, "error": "Could not retrieve original message"}
            ...
            if original.get("thread_id"):
                ...
                if thread_result.get("success"):
                    thread_messages = thread_result.get("data", {}).get("messages", [original])
            ...
            draft_result = await self.llm_client.draft_email_reply(thread_content, intent, sender_name)
            
            return {
                "success": True,
                "data": {
                    "draft": draft_result.get("reply_body", ""),
                    "subject": draft_result.get("subject", f"Re: {original.get('subject', '')}"),
    ...
    async def summarize_emails(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    async def extract_action_items(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    @capability(
        ...
        parameters=[
            ParameterSchema(
                ...
                required=True,
                enum=["mark_read", "mark_unread", "archive", "delete", "star", "unstar", "add_labels", "remove_labels"]
            ),
            ParameterSchema(
                name="message_ids",
                type="array",
    ...
    async def manage_emails(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            for msg_id in message_ids:
                ...
                results.append({"message_id": msg_id, "success": result.get("success", False)})
    ...
    async def download_attachments(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await self.gmail_client.download_email_attachments(message_id, user_id)
    ...
    async def resolve_message_ids(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "data": {
                    ...
                    "resolution_rate": len(resolved) / len(identifiers) if identifiers else 0
                },
