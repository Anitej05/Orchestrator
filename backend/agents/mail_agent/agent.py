...
class CentralAgent:
    ...
    async def search(
        self, query: str, max_results: int, user_id: str
    ) -> Dict[str, Any]:
    ...
    async def summarize_emails(self, request: SummarizeRequest) -> Dict[str, Any]:
        ...
        if not target_ids:
            return {
                "success": True,
                "data": {"summary": "No emails found.", "sources": [], "count": 0},
            }
        ...
        if not all_bodies:
            return {
                "success": True,
                "data": {
                    "summary": "Could not retrieve content.",
                    "sources": [],
                    "count": 0,
                },
            }
        ...
        return {
            "success": True,
            "data": {"summary": final_summary, "sources": all_sources},
        }
    ...
    async def extract_actions(
        self, request: ExtractActionItemsRequest
    ) -> Dict[str, Any]:
        user_id = request.user_id
        target_ids = request.message_ids
        if not target_ids and request.use_history:
        ...
        for i in range(0, len(target_ids), 15):
            emails = await self.gmail.batch_fetch_emails(
                target_ids[i : i + 15], user_id
            )
            all_texts.extend(
        ...
        if prompt and not action:
            ...
            for step in plan.steps:
                ...
                if "search" in step_action:
                    res = await central_agent.search(
                    ...
                    res = await central_agent.summarize_emails(
            ...
            last_canvas = None
            for r in results:
                if isinstance(r.get("result"), dict) and r["result"].get(
                    "canvas_display"
                ):
