...
def unpack(
    ...
    """Unpack an Office file into a directory of XML files.

    Args:
        input_file: Path to the Office file (.docx, .pptx, .xlsx)
        output_directory: Directory to extract files into
    ...
    try:
        ...
        # Escape smart quotes so they survive XML editing
        for xml_file in xml_files:
...
def _pretty_print_xml(xml_file: Path) -> None:
    """Pretty-print an XML file for human readability."""
    try:
        content = xml_file.read_text(encoding="utf-8")
        dom = minidom.parseString(content)
        xml_file.write_bytes(dom.toprettyxml(indent="  ", encoding="utf-8"))
    except Exception:
        pass  # Skip non-XML files or malformed XML
