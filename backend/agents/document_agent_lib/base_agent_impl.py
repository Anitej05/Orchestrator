...
class DocumentAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "document_agent", services: Optional[AgentServices] = None):
        super().__init__(
            agent_id=agent_id,
    ...
    async def _get_custom_metrics(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            "sessions": len(self.session_manager._sessions) if hasattr(self.session_manager, '_sessions') else 0,
    ...
    def _validate_edit_plan(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        allowed = {
            "add_paragraph", "add_heading", "format_text", "replace_text",
            "add_table", "add_content", "replace_content", "delete_content",
            "add_image", "modify_style", "convert_format"
        }
    ...
    def _execute_edit_action(self, editor: DocumentEditor, action: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if action_type == 'add_paragraph':
                result = editor.add_paragraph(action.get('text', ''), action.get('style', 'Normal'))
            elif action_type == 'add_heading':
                result = editor.add_heading(action.get('text', ''), action.get('level', 1))
            elif action_type == 'format_text':
                result = editor.format_text(action.get('text', ''), **action.get('options', {}))
            elif action_type == 'replace_text':
                result = editor.replace_text(action.get('old_text', ''), action.get('new_text', ''))
    ...
    async def analyze_document(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    async def create_document(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    async def edit_document(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check risk
            if (risk.get('risk_score', 0) >= 0.6 or risk.get('intent') in {'destructive', 'overwrite'}) and not auto_approve:
                return {
                    "success": False,
    ...
    async def get_versions(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    async def undo_redo(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            current_idx = self.version_manager.index.get(doc_key, {}).get('current_version', -1)
            
            if action == "undo":
                if current_idx > 0:
                    target_version = versions[current_idx - 1]['version_id']
                    success = self.version_manager.restore_version(file_path, target_version)
                    return {
                        "success": success,
    ...
    async def extract_data(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        if not file_path or not extraction_type:
            return {"success": False, "error": "file_path and extraction_type are required"}
        ...
        try:
            ...
            if extraction_type == "tables":
                ...
                extraction_params = {
                    ...
                    "all": {"extract_summary": True, "extract_entities": True, "extract_key_values": True}
                }
                params = extraction_params.get(extraction_type, extraction_params["all"])
                extracted = await self.llm_client.extract_structured_data(content, **params)
    ...
    async def display_document(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    async def compare_documents(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            comparison = await self.llm_client.analyze_document_with_query(combined_content, comparison_query)
