...
class SpreadsheetAgent(BaseAgent):
    ...
    def __init__(self, agent_id: str = "spreadsheet_agent", services: Optional[AgentServices] = None):
        super().__init__(
            agent_id=agent_id,
            agent_name="Spreadsheet Agent",
            services=services,
            config=SpreadsheetAgentConfig()
        )
    ...
    def _build_canvas(
        ...
        try:
            ...
            canvas_dict = display.model_dump() if hasattr(display, 'model_dump') else display.dict()
    ...
    async def load_file(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    async def upload_file(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            # Load the file
            df, detection_info = await self.client.load_file(content=content, filename=filename)
    ...
    async def export_file(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        """Export data to file."""
    ...
    async def process_data(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        if not file_id or file_id not in session.dataframes:
            return {"success": False, "error": "No data loaded. Please load a file first."}
        ...
        try:
            ...
            canvas = self._build_canvas(df, "Processed Data", file_id) if df_modified else None
            ...
            return {
                ...
                "canvas_display": canvas,
                "message": f"Data processed: {computed_answer[:100]}..." if len(str(computed_answer)) > 100 else f"Data processed: {computed_answer}"
            }
    ...
    async def transform_data(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    async def filter_data(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    async def sort_data(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    @capability(
        ...
        parameters=[
            ...
            ParameterSchema(
                ...
                required=True,
                enum=["sum", "mean", "count", "min", "max", "nunique", "std", "var", "median"]
            ),
            ParameterSchema(
                name="file_id",
                type="string",
    ...
    async def aggregate_data(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if not group_cols:
                return {"success": False, "error": f"Group column not found: {group_by}"}
            if not agg_cols:
                return {"success": False, "error": f"Aggregate column not found: {agg_column}"}
            ...
            agg_funcs = {
                ...
                "min": "min", "max": "max", "nunique": "nunique",
    ...
    async def merge_data(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        ...
        if len(file_ids) < 2:
            return {"success": False, "error": "At least 2 file_ids are required for merge"}
    ...
    async def add_column(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
    ...
    async def drop_column(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        """Drop a column."""
        column = params.get("column")
        thread_id = params.get("thread_id", "default")
        file_id = params.get("file_id")
        ...
        try:
            ...
            canvas = self._build_canvas(result_df, f"Dropped Column: {cols[0]}", file_id)
            
            return {
                "success": True,
                "data": {"dropped": cols[0], "remaining_columns": len(result_df.columns)},
                "canvas_display": canvas,
    ...
    async def rename_column(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        """Rename a column."""
        ...
        try:
            ...
            canvas = self._build_canvas(result_df, f"Renamed: {cols[0]} → {new_name}", file_id)
    ...
    async def fill_missing(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        """Fill missing values."""
        column = params.get("column")
        ...
        try:
            ...
            return {"success": False, "error": f"Failed to fill missing values: {str(e)}"}
    ...
    async def get_summary(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        """Get data summary."""
        thread_id = params.get("thread_id", "default")
        file_id = params.get("file_id")
        ...
        try:
            ...
            summary = {
                ...
                "null_percentage": {k: round(v/len(df)*100, 2) for k, v in df.isnull().sum().to_dict().items() if v > 0}
            }
            ...
            if categorical_cols:
                summary["categorical_counts"] = {col: df[col].nunique() for col in categorical_cols[:5]}
    ...
    async def list_files(self, params: Dict[str, Any], context: ExecutionContext) -> Dict[str, Any]:
        """List all files in session."""
        thread_id = params.get("thread_id", "default")
        
        session = self.state.get(thread_id)
        if not session:
            return {"success": True, "data": {"files": [], "count": 0}, "message": "No files in session"}
