...
class SpreadsheetAgent(SpreadsheetLLMHelpers):
    ...
    async def _execute_complex(
        ...
        for attempt in range(max_retries):
            ...
            if plan.needs_clarification:
                ...
                self.state.pause_task(thread_id, task_id, plan.question, {
    ...
    async def _execute_step_by_name(
        ...
        if action == 'load_file':
            ...
            return await self._step_export(params, df, session, thread_id)
        elif action == 'process':
            df = self._get_current_df(session)
            return await self._step_process(params, df, session, thread_id)
        else:
            # Route all other actions through process
            instruction = params.get('instruction') or params.get('question') or action
            df = self._get_current_df(session)
            return await self._step_process({'instruction': instruction}, df, session, thread_id)
