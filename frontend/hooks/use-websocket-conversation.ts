      ...
      ws.current.onmessage = (event) => {
        try {
              ...
              const updatedTaskStatuses = {
                ...
                [taskName]: {
                  ...
                  result: typeof result === 'string'
                    ...
                    : (result ? '[Result data available]' : undefined),
                }
              };
          ...
          // === OMNI-DISPATCHER HANDLERS ===
            ...
            console.log('Live canvas update received');
            const currentState = useConversationStore.getState();
            _setConversationState({
              has_canvas: eventData.data.has_canvas,
              canvas_type: eventData.data.canvas_type,
              // Don't store full canvas_content during live updates - too large
              // Only store essential view data, truncate if too large
              browser_view: eventData.data.browser_view?.length > 500000
                ? undefined  // Skip if over 500KB to prevent memory issues
                : eventData.data.browser_view,
          ...
          // We use this as the single source of truth to update our store.
            ...
            try {
              ...
              if (finalState.has_canvas && (finalState.canvas_content || (finalState as any).canvas_data) && finalMessages.length > 0) {
                const lastMessage = finalMessages[finalMessages.length - 1];
                if (lastMessage.type === 'assistant') {
                  lastMessage.canvas_content = finalState.canvas_content;
                  (lastMessage as any).canvas_data = (finalState as any).canvas_data;
              ...
              _setConversationState({
                ...
                plan: finalState.plan || (finalState as any).task_plan || currentState.plan || [],
                ...
                needs_canvas: (finalState as any).needs_canvas !== undefined ? (finalState as any).needs_canvas : (currentState as any).needs_canvas,
                // MEMORY CLEANUP: Clear browser_view on completion to free memory
