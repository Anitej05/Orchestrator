...
export default function AgentPreview({ formData }: AgentPreviewProps) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 p-6 sticky top-24">
      <h2 className="text-xl font-bold text-blue-600 dark:text-blue-400 mb-6">Live Preview</h2>

      {/* Preview Card */}
      <div className="bg-white dark:bg-gray-700 rounded-lg border-2 border-blue-200 dark:border-blue-800 p-6 hover:shadow-xl hover:border-blue-400 dark:hover:border-blue-600 transition-all duration-200">
        {/* Header */}
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">{displayFramework}</p>
            </div>
          </div>
        </div>
        ...
        {/* Description */}
        <p className="text-sm text-gray-600 dark:text-gray-300 mb-4 line-clamp-3">{displayDescription}</p>

        {/* Endpoints Preview */}
        {formData.endpoints.length > 0 && (
          <div className="mb-4">
            <h4 className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-2">Endpoints:</h4>
            <div className="space-y-1">
              {formData.endpoints.slice(0, 2).map((endpoint, index) => (
                <div key={`${endpoint.endpoint}-${index}`} className="flex items-center text-xs text-gray-500 dark:text-gray-400">
        ...
        {/* Preview Note */}
        <div className="text-xs text-gray-400 dark:text-gray-500 italic">This is how your agent will appear in the marketplace</div>
      </div>
    </div>
  )
}
