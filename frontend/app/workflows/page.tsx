...
export default function WorkflowsPage() {
  ...
  const handleExecute = async (workflow: Workflow) => {
    setExecuting(true);
    try {
      // Call create-conversation endpoint to create a new conversation with the plan
      const response = await authFetch(
        `http://localhost:8000/api/workflows/${workflow.workflow_id}/create-conversation`,
        { 
          method: "POST",
      ...
      // Redirect to home page with threadId to show the plan for review
      if (data.thread_id) {
        window.location.href = `/?threadId=${data.thread_id}`;
      }
    } catch (error) {
      console.error("Failed to create workflow conversation:", error);
      setExecuting(false);
    }
  };
                  ...
                  <div className="flex items-center justify-between text-sm text-gray-500">
                    ...
                    {workflow.next_scheduled_run && (
                      <div className="text-xs text-blue-600" title="Next scheduled run">
                        Next: {new Date(workflow.next_scheduled_run).toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })} IST
                      </div>
                    )}
                  </div>
